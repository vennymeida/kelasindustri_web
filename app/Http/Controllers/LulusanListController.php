...
class LulusanListController extends Controller
{
    ...
    {
        $lulusan->load(['profileKeahlians.keahlian']);

        if ($lulusan && $lulusan->ringkasan) {
            $lulusan->ringkasan = Str::replace(
                ['<ol>', '</ol>', '<li>', '</li>', '<br>', '<p>', '</p>'],
                ['', '', '', ", ", '', '', "\n"],
                $lulusan->ringkasan
            );
            $lulusan->ringkasan = rtrim($lulusan->ringkasan, ', ');
        }

        return view('lulusan.view', compact('lulusan'));
    }
}
